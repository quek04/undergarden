					...
					LootPool.lootPool()
						.add(
							LootItem.lootTableItem(UGItems.UNDERBEANS)
								...
								.when(
									LootItemBlockStatePropertyCondition.hasBlockStateProperties(UGBlocks.UNDERBEAN_BUSH.get())
										.setProperties(StatePropertiesPredicate.Builder.properties().hasProperty(UnderbeanBushBlock.AGE, 3))
								)
						)
				)
				.withPool(
					LootPool.lootPool()
						.add(LootItem.lootTableItem(UGItems.UNDERBEANS).apply(SetItemCountFunction.setCount(UniformGenerator.between(1.0F, 2.0F))))
				)
		);
