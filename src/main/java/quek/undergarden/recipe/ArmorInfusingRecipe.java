...
public class ArmorInfusingRecipe implements Infusing {
	...
	public ArmorInfusingRecipe(/*String group, */InfusingBookCategory category/*, Ingredient ingredient, ItemStack result, float experience, int infusingTime, boolean isUtheriumFuel*/) {
	...
	public boolean matches(InfuserRecipeInput input, Level level) {
		return input.itemStack().getItem() instanceof ArmorItem && !input.isUtheriumFuel()/* && this.utheriumFuel == input.isUtheriumFuel()*/;
	}

	@Override
	public ItemStack assemble(InfuserRecipeInput input, HolderLookup.Provider registries) {
		return input.itemStack().copy();
	}
	...
	public RecipeSerializer<?> getSerializer() {
		return UGRecipeSerializers.ARMOR_INFUSING.get();
	}

	@Override
	public RecipeType<?> getType() {
		return UGRecipeTypes.ARMOR_INFUSING.get();
	}
	...
	public static class Serializer implements RecipeSerializer<ArmorInfusingRecipe> {

		private final MapCodec<ArmorInfusingRecipe> codec;
		private final StreamCodec<RegistryFriendlyByteBuf, ArmorInfusingRecipe> streamCodec;
		...
		public Serializer() {
			this.codec = RecordCodecBuilder.mapCodec(
				...
				).apply(instance, ArmorInfusingRecipe::new)
			);
			this.streamCodec = StreamCodec.of(this::toNetwork, this::fromNetwork);
		}

		@Override
		public MapCodec<ArmorInfusingRecipe> codec() {
			return this.codec;
		}

		@Override
		public StreamCodec<RegistryFriendlyByteBuf, ArmorInfusingRecipe> streamCodec() {
			return this.streamCodec;
		}

		private ArmorInfusingRecipe fromNetwork(RegistryFriendlyByteBuf buf) {
			...
			return new ArmorInfusingRecipe(/*group, */category/*, ingredient, result, experience, infusingTime, isUtheriumFuel*/);
		}

		private void toNetwork(RegistryFriendlyByteBuf buf, ArmorInfusingRecipe recipe) {
