...
public class UGTeleporter implements ITeleporter {
        ...
        for(int k1 = -1; k1 < 3; ++k1) {
            for(int i2 = -1; i2 < 4; ++i2) {
                if (k1 == -1 || k1 == 2 || i2 == -1 || i2 == 3) {
                    ...
                    this.world.setBlock(mutablePos, Blocks.STONE_BRICKS.defaultBlockState(), 3);
                }
            }
        }

        BlockState undergardenPortal = UGBlocks.UNDERGARDEN_PORTAL.get().defaultBlockState().setValue(UndergardenPortalBlock.AXIS, axis);

        for(int j2 = 0; j2 < 2; ++j2) {
            for(int l2 = 0; l2 < 3; ++l2) {
                mutablePos.setWithOffset(blockpos, j2 * direction.getStepX(), l2, j2 * direction.getStepZ());
                this.world.setBlock(mutablePos, undergardenPortal, 18);
            }
        }
    ...
    public PortalInfo getPortalInfo(Entity entity, ServerWorld destWorld, Function<ServerWorld, PortalInfo> defaultPortalInfo) {
        boolean destinationIsUG = destWorld.dimension() == UGDimensions.UNDERGARDEN_WORLD;
