...
public class ExampleBiome extends Biome {
    ...
    public ExampleBiome() {
        super((new Builder()).surfaceBuilder(new DefaultSurfaceBuilder(
                SurfaceBuilderConfig::deserialize), new SurfaceBuilderConfig(Blocks.DIAMOND_BLOCK.getDefaultState(), Blocks.DIAMOND_BLOCK.getDefaultState(), Blocks.DIAMOND_BLOCK.getDefaultState())).precipitation(RainType.RAIN).category(Category.NONE).depth(0.1F).scale(0.2F).temperature(2.0F).downfall(0).waterColor(342306).waterFogColor(342306).parent(null));
